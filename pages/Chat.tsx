...
export const Chat: React.FC = () => {
  ...
  // Listener global: incrementar badge de n\u00e3o lidas quando uma nova msg chega em um canal que N\u00c3O est\u00e1 aberto
  useEffect(() => {
    if (!userId) return;
    ...
    const globalSub = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          // Só incrementar se NÃO for minha mensagem e NÃO estiver com o canal aberto
          if (newMsg.sender_id !== userId && newMsg.channel_id !== selectedChannelId) {
            setChannels(prev =>
              prev.map(ch =>
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    // Atualização otimista: exibir imediatamente na tela
              ...
              <div>
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-xs text-slate-500 dark:text-slate-400">
                    Ocupado &bull; {selectedChannel.type === 'group' ? 'Grupo' : 'Mensagem Direta'}
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-4 bg-slate-50/50 dark:bg-slate-950/50">
            {currentMessages.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-slate-400">
                <p className="text-sm">Inicie a conversa em {selectedChannel.name}</p>
              </div>
            ) : (
              currentMessages.map((msg) => (
                ...
                >
                  <div className={`shrink-0 w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${msg.isMe
                    ...
                    {msg.isMe ? 'EU' : selectedChannel.name.substring(0, 2).toUpperCase()}
                  </div>

                  <div className={`group relative p-3 rounded-2xl shadow-sm text-sm ${msg.isMe

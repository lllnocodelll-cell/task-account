        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
           ...
           <div className="md:col-span-1">
             <Select 
                label="Tarefa"
                options={[
                   { value: 'fiscal', label: 'Fechamento Fiscal' },
                   { value: 'folha', label: 'Folha de Pagamento' },
                   { value: 'contabil', label: 'Fechamento Contábil' },
                   { value: 'das', label: 'DAS' },
                ]}
                defaultValue={initialData?.taskName === 'Fechamento Fiscal' ? 'fiscal' : 'das'}
             />
           </div>
           <Select 
             label="Responsável" 
             ...
             defaultValue={initialData?.responsible === 'Ana Souza' ? '1' : '2'}
           />
           <Select 
             label="Setor"
             options={[
                     ...
                     {SIMPLES_ANNEXES.map((annex) => (
                        <button
                           ...
                           className={`flex items-center gap-2 px-4 py-2 rounded-lg border text-sm font-medium transition-all ${
                              selectedAnnexes.includes(annex)
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
                   {DF_MODELS.map((model) => (
                      <div 
                         ...
                         title={model.desc}
                         className={`cursor-pointer p-4 rounded-lg border-2 text-center transition-all duration-200 relative group ${
                            selectedDfe.includes(model.id)

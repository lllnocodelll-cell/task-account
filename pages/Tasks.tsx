...
const KanbanColumn: React.FC<KanbanColumnProps> = ({
  ...
  const localFilteredTasks = tasks.filter(task => {
    ...
    const hasTagMatch = !colFilter.hasTag || !!task.temporary_tag;
            ...
            <div className="bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border border-slate-200/80 dark:border-slate-700/60 rounded-2xl shadow-2xl ring-1 ring-black/5 dark:ring-white/5 overflow-hidden">
              ...
              <div className="px-3.5 py-3 space-y-1 overflow-y-auto custom-scrollbar" style={{ maxHeight: filterCoords.maxPanelH - 120 }}>
                ...
                <div>
                  ...
                  >
                    <Building2 size={11} className="text-blue-500 dark:blue-400 shrink-0" />
                ...
                <div>
                  ...
                  >
                    ...
                    {(colFilter.taxRegime || colFilter.competence !== '' || colFilter.dueDate || colFilter.noMovement || colFilter.exceededSublimit || colFilter.notifiedExclusion) && (
                      <span className={`w-1.5 h-1.5 rounded-full ${accent.dot} shrink-0`} />
                    )}
                    {expandedFilterSections.fiscal
                      ? <ChevronDown size={11} className="text-slate-400 shrink-0" />
                  ...
                  {expandedFilterSections.fiscal && (
                    <div className="space-y-2 mt-2 px-1 pb-2 animate-in fade-in slide-in-from-top-1 duration-200">
                      ...
                      <div className="flex flex-wrap gap-1.5 pt-1">
                        ...
                        <button
                          onClick={() => setColFilter(p => ({ ...p, hasTag: !p.hasTag }))}
                          className={`flex-1 flex items-center justify-between px-2 py-1.5 rounded-lg border transition-all text-[10px] font-bold ${colFilter.hasTag ? 'bg-indigo-50 dark:bg-indigo-500/15 border-indigo-300 dark:border-indigo-500/40 text-indigo-600 dark:text-indigo-400' : 'bg-slate-50 dark:bg-slate-800/50 border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-500'}`}
                        >
                          <span>Tags</span>
                          {colFilter.hasTag && <span className="ml-1">✓</span>}
                        </button>
                      </div>
                    </div>
                  )}
        ...
        {localFilteredTasks.map(task => {
          ...
          return (
            ...
            >
              ...
              <div className="px-3 pb-2 flex flex-wrap items-center justify-start gap-2">
    ...
    if (isRecurring && task.clientId) {
      try {
        const { count, error } = await supabase
          ...
          .gt('competence', task.competence);
          
        if (!error && count && count > 0) {
          hasFutureTasks = true;
        }
      } catch (e) {
        console.error('Erro ao verificar tarefas futuras na exclusão:', e);
      }
    }

    if (isRecurring && hasFutureTasks && !hasDocs) {
                        ...
                        <th className={`px-6 py-4 align-top min-w-[180px] ${tarefaActive ? 'relative z-50' : 'relative z-10'}`}>
                          <div className="flex items-center justify-between gap-2 h-6">
                            ...
                            <TableColumnFilter label="Tarefa" isActive={tarefaActive} activeCount={tarefaCount}>
                              ...
                              {/* Toggle Com TAG */}
                              <button
                                onClick={() => setFilters(p => ({ ...p, hasTag: !p.hasTag }))}
                                className={`w-full flex items-center justify-between px-3 py-2 rounded-lg border transition-all text-[11px] font-semibold ${filters.hasTag ? 'bg-indigo-50 dark:bg-indigo-500/10 border-indigo-200 dark:border-indigo-500/30 text-indigo-700 dark:text-indigo-400' : 'bg-slate-50 dark:bg-slate-800/50 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400'}`}
                              >
                                <span>Com TAG</span>
                                <span className={`w-4 h-4 rounded-full flex items-center justify-center text-[9px] font-black ${filters.hasTag ? 'bg-indigo-500 text-white' : 'bg-slate-200 dark:bg-slate-700 text-slate-500'}`}>
                                  {filters.hasTag ? '✓' : ''}
                                </span>
                              </button>
                              {/* Limpar */}
                              {tarefaActive && (
                                <button onClick={() => { handleFilterChange('taskName', ''); setFilters(p => ({ ...p, noMovement: false, hasTag: false })); }} className="w-full text-center text-[10px] font-semibold text-rose-500 dark:text-rose-400 hover:text-rose-700 pt-1">
                                  Limpar filtros
                                </button>
                              )}
                            </TableColumnFilter>
        ...
        {selectedClientForDetails && (
          <ClientForm
            onBack={() => {
              ...
              fetchClients(); // Refresh clients list to update drawer details
            }}

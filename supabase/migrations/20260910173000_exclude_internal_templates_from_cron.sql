-- Migration: 20260910173000_exclude_internal_templates_from_cron.sql
-- Description: Garantir que process_automated_chat_templates() nunca processe templates internos (target_audience = 'internal').
...
BEGIN
    ...
    -- Iterar sobre os agendamentos cadastrados onde o modelo tem is_automated = true, target_audience = 'external' e o horário corrente coincide
    FOR t IN 
        SELECT 
            s.id AS schedule_id,
            s.trigger_type,
        ...
        AND (tmpl.target_audience IS NULL OR tmpl.target_audience = 'external')
